...
install_dependency(){
   if ! test -x "$1" ; then
      $guitool --info --text=$"We need to install a dependency package, please ensure that you have access to the internet to continue"
      if ! check_if_internet ; then
         $guitool --error --text=$"No internet connection found"
         return 1
      else
...
do_check_installed_packages(){
   $guitool --question --text=$"Do you want to check your installed packages? Your system can run wrongly if it doesn't has installed all the default packages included by Elive, it is very recommended to do this check. Do you want to continue?" || return

   dpkg -l | awk '{print $1" "$2}' | grep -E "(^ii|^hi)" | awk '{print $2}' > /tmp/.installed-packages

   if ! check_if_internet ; then
      $guitool --error --text=$"No internet connection found. To run this tool you need to be connected to internet"
      return 1
   else
      urxvt -e bash -c "apt-get update"
   fi

   ( sleep 10 ; echo 25 ; sleep 10000 ) | $guitool --progress --pulsate --text=$"Checking packages, please be patient..." &
   gui_pid=$!

   cat /etc/elive/system/packages/packages-installed | while read package
   do
      if ! grep -q $package /tmp/.installed-packages ; then
         if $guitool --question --text=$"Warning: I have found a package that is not installed in your system, this is a default package of your Elive system, your system may work incorrectly without it, do you want to reinstall it?""\n\n"$"Package: ${package}" ; then
...
#===============================================================================
do_health(){
   $guitool --question --text=$"This tool allows you to do some hardware verification, to see if the components work properly and without problems. These tests check for errors in the hard disk, in your RAM, and your processor. If you complete all these steps without any error, your computer is working perfectly. Do you want to continue ?" || return 0

   #-------------------------------------------------------------------------------
   #   Disk check
   #-------------------------------------------------------------------------------
   if $guitool --question --text=$"Do you want to verify if your hard disk contains bad blocks ? This operation can be very slow and time-consuming. If you see several lines with lots of numbers, then errors were found, but you can stop the process at any moment by closing the terminal"   # FIXME: add a feature in the installer to allow bad blocks
   then
      result="$( fdisk -l 2>/dev/null | grep "^Disk" | grep bytes | grep -v "contain a valid partition table" | awk '{print $2"\n"$3" "$4}' | sed -e 's|:||g' -e 's|,||g' \
            | awk '{print $0}END{print"Select_manually\nManual"}' | $guitool --list --text=$"Select the Hard Disk to use" --column=$"Disk" --column=$"Size" || echo cancel )"

      if [[ "$result" = "Select_manually" ]] ; then
         result="$( $guitool --entry --text=$"Enter the name of the desired device." --entry-text="/dev/..." || echo cancel )"
      fi
      if ! check_result_guitool $result ; then
         $guitool --error --text=$"Incorrect option selected"
         unset result
      fi
      if ! test -b $result 1>/dev/null 2>&1 ; then
         $guitool --error --text=$"Incorrect disk selected"
         unset result
      fi
   ...
   #   Ram check 
   #-------------------------------------------------------------------------------
   $guitool --info --text=$"To run the RAM diagnostic check, you need to reboot the computer and select that option from the boot menu:"" Memory RAM Diagnostic. "$"You need to run this process for a few hours, better yet, run it overnight. If you see any error there you should change your RAM immediately, RAM errors are really very bad for your data and is the common cause of lots of errors that do not seem to make sense. By the way be sure that you do not get 'false positives' errors (Search online for help)"

   
   #-------------------------------------------------------------------------------
   #   Other checks
   #-------------------------------------------------------------------------------
   $guitool --info --text=$"It is recommended to open the computer case from time to time to check that everything is alright, i.e. there's no dust on the coolers and that all the pieces are correctly attached or well-seated."
}
...
#===============================================================================
do_cleanup_system(){
   if $guitool --question --text=$"The logs of your system sometimes takes a large quantity of space, specially with big debug options enabled, you can also disable it in the installation process of Elive (complete mode)"\n\n""$"The actual size for it is: ""$(du -hs /var/log/ )" ; then
   ...
   if $guitool --question --text=$"Do you want to clean the trash and temporal files of all the users?" ; then
      for user in $DHOME/*
      do
   ...
   if $guitool --question --text=$"Do you want to run an application to inspect your hard disk searching where more space is wasted?" ; then
...
#===============================================================================
do_boot_recover(){
   $guitool --question --text=$"You are going to recover the boot of the normal system, do you want to continue ?" || return 0
   ...
   if $guitool --question --text=$"Do you want to reinstall Grub to your disk boot ? (you should not need this)" ; then
      result="$( fdisk -l 2>/dev/null | grep "^Disk" | grep bytes | grep -v "contain a valid partition table" | awk '{print $2"\n"$3" "$4}' | sed -e 's|:||g' -e 's|,||g' \
            | awk '{print $0}END{print"Select_manually\nManual"}' | $guitool --list --text=$"Select the Hard Disk to use" --column=$"Disk" --column=$"Size" || echo cancel )"
...
do_entrance_conf_recover(){
   $guitool --question --texty=$"The Entrance (login manager) configuration is messed up. Do you want to revert to the original default Elive configuration?" && cp /etc/entrance_config.cfg.bak /etc/entrance_config.cfg
   sleep 1
   $guitool --info
}
...
do_kernel_install_new(){
   ...
   $guitool --question --text=$"You are going to install a new Elive Kernel. The old one will be saved, with the new you can have updated drivers, you can install special versions of the kernel like the highmem that let you to use more than 4 GB of memory RAM. Do you want to continue ?" || return 0
  ...
  for package in $( apt-cache search linux-image elive | grep "^linux-image" | sort -g | awk '{print $1}' )
      do
         ...
         case "$package" in
            (*core2*highmem*)
               echo $"For Core2Duo processors and more than 4 GB of RAM"
            ;;
            (*highmem*|*bigmem*)
               echo $"Lets you use more than 4 GB of RAM"
            ;;
            (*)
               echo $"Normal Version"
            ;;
         esac
      done | $guitool --list --column=$"Package" --column=$"Description" --width=630 --height=400 || echo cancel )"
   ...
   if test -f /etc/elive/system/packages/packages-to-hold ; then
      if grep -q fglrx-glx /etc/elive/system/packages/packages-to-hold ; then
         $guitool --question --text=$"Warning: You have fglrx (ATI) packages installed. Installing a new kernel may need you to update the fglrx-glx package, which can make an incompatible fglrx in your current kernel. You are then limited to using the new one, and for revert (downgrade) to the original kernel you will need to reinstall Elive. Do you wish to continue ?" || return 0
         hold_all=yes
...
do_wifi_configure(){
   ...
   ndiswrapper -l | $guitool --list --column="ID" --text=$"This is a list of installed drivers"

   $guitool --info --text=$"If the driver for your wifi is not installed you can try to download a different version of the driver from internet or just select to scan your windows partition in order to search the files for the installed driver"

   lsmod | awk '{print $1}' | grep -q b43 && rmmod b43
   sync
   modprobe ndiswrapper

   $guitool --question --text=$"It is your wireless card working now ? Try it in the internet configuration and if not works, click to the Cancel button (otherwise we will install it permanently)" || return
...
#===============================================================================
do_remove_win_apps(){
   $guitool --question --text=$"This tool will remove entirely all of your windows applications installed. If you want to remove them one by one, just use the deinstallers from the menus of applications"

   result="$( ls -1 $DHOME | $guitool --list --column=$"User" --text=$"Select the desired user" || echo cancel )"
...
#===============================================================================
do_remove_menu_apps(){
   $guitool --question --text=$"This tool is for removal of the application entries of your menus, sometimes they are badly written and you may need to remove them in order to have them working correctly. Do you want to continue?" || return
   result="$( ls -1 $DHOME | $guitool --list --column=$"User" --text=$"Select the desired user" || echo cancel )"
   ...
   if [[ -d "${DHOME}/${result}/.local/share/applications" ]] ; then
      ...
      if [[ ! -z "$resultlist" ]] ; then
         result="$( echo "$resultlist" | $guitool --list --column="Remove" --column="Filename" --column="Name" --column="Comment" --text=$"Select menu entry's of applications to remove" --width=600 --height=400 --checklist )"
      else
         $guitool --info --text=$"You don't have any entries remaining to remove."
         return 0
      fi
...
do_update_configurations(){
   $guitool --question --text=$"This tool is to reset the user configurations to the defaults set by Elive, your configuration for such application will be erased, do you want to continue?" || return
...
do_recover_configuration(){
   $guitool --question --text=$"This tools is for recovery of an old updated configuration, there's no history so you can recover it only to the last updated one. Do you want to continue?" || return
...
#===============================================================================
do_disks_plug_fix(){
   $guitool --question --text=$"This tool will allow you to remove fstab entry's, sometimes you may have an entry on your fstab that doesn't allow you to automatically mount a plugged device, by removing it you will have it pluggable again"
   ...
   if [[ ! -z "$resultlist" ]] ; then
      result="$( echo "$resultlist" | $guitool --list --column=$"Device" --column=$"Size" --column=$"Type" || echo cancel )"
   else
      $guitool --info --text=$"You don't have any entries remaining to remove."
      return 0
   fi
...
"Exit\n"$"Exit from here" \
   ...
   DisksPlug)
      ...
      ;;
   Report)
      $guitool --info --text=$"If you found a malfunction in Elive, just report it to http://bugs.elivecd.org. If you want to discuss it with other users you can use the forum http://forum.elivecd.org or chat with Elive users using the application from the Internet applications menu (it should also be in your dock as a second icon)."
      ;;
   WinMigrate)
      $guitool --info --text=$"For the Web Browser (firefox): From your windows go to 'export bookmarks' and in your Elive do a 'import bookmarks', if you don't have firefox installed on windows just install it"
      $guitool --info --text=$"For emule: Just copy or move entirely the directory Incoming to the amule of Elive (you may need to install it)"
      $guitool --info --text=$"For torrents: You need to add again the torrent files to the new application, for continue your downloads just copy the data of your semi-downloaded torrents to the new application temporal files"
      ;;
   Translations)
      $guitool --info --text=$"You are welcome to collaborate with Elive by making translations, for that you just need to run the application 'eltrans', remember that you do not need to do the translations entirely, so that a small part of the work done by many users results in a big end result. Remember too that the English translations are the most important ones, in order to translate the Elive system to a more correct english just do the translations to the 'en' language, remember too that all the other translations are based on these english ones. Thanks a lot for your collaboration."
      ;;
   cancel|Exit)
      return 0
      ;;
